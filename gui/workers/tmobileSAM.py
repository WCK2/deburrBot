...
class TMobileSAM(QThread):
    ...
    def run(self, input_points:list, input_labels:list):
        try:
            ...
            if np.array_equal(origin, [0, 0, 0]):
                raise ValueError(f'Invalid origin: {origin} calculated for the best fit plane.')
            ...
            if tool_x_diameter_pixels <= 0:
                raise ValueError(f'Invalid tool_x_diameter_pixels: {tool_x_diameter_pixels} calculated prior to generating coordinate_pairs.')
