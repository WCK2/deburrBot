...
class CUSTOM_PATH(NFrame):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.setObjectName('page_custom_path')
        ...
        tcustom.set_finished.connect(self.__tcustom_set_finished)
        tcustom.run_finished.connect(self.__tcustom_run_finished)
    ...
    def __onimgbtn(self):        
        ...
        if self.btn_new_picture.isEnabled() and tcustom.image_is_set_flag:
            self.btn_generate.setEnabled(True)

    def __on_new_picture(self):
        if self.thread_camera.threadstatus(): return
        if tcustom.threadstatus(): return
    ...
    def __tcustom_set_finished(self):
        self.loading_gif.hide()
        if self.star_objs:
            self.btn_generate.setEnabled(True)
    ...
    def __on_generate(self):
        self.btn_start.setEnabled(False)
        tcustom.run(self.star_coords, self.star_labels)
    ...
    def __tcustom_run_finished(self):
        self.__setupimg(tcustom.draw_img)
        if tcustom.target_pairs_post_status:
    ...
    def __on_start(self):
        if tcustom.target_pairs_post_status:
            ...
            self.btn_start.setEnabled(False)
        # TODO reset buttons ... and img? idk reset tcustom.target_pairs_post_status?
