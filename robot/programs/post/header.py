...
#~ Force move stuff
def force_target_run(targets: list, zcontact: bool=True, easeon: int=20, easeoff: int=40):
...
def force_target_pair_run(t_start: list, t_end: list, zcontact: bool=True, easeon: int=20, easeoff: int=40):
    """ Target pair pathing in servo mode (from t_start to t_end only) """
    data_log = {
        'pose': [],
        'zero_forces': [],
        'error': [],
...
def generator1_deburring_program():
    ...
    # target_pairs = [[[-48.391, -112.611, -27.531, 0.0, 0.0, 0.0], [-48.571, -249.563, -30.631, 0.0, 0.0, 0.0]], [[-48.571, -249.563, -30.631, 0.0, 0.0, 0.0], [49.18, -240.672, -20.638, 0.0, 0.0, 0.0]], [[49.18, -240.672, -20.638, 0.0, 0.0, 0.0], [42.45, -114.815, -10.438, 0.0, 0.0, 0.0]], [[42.45, -114.815, -10.438, 0.0, 0.0, 0.0], [-7.895, -179.8, -22.734, 0.0, 0.0, 0.0]]]
...
def test_single_contour_run():
    ...
    targets = mem.generator1_target_pairs ## change var
    targets = [[-48.391, -112.611, -27.531, 0.0, 0.0, 0.0], [-48.571, -220.563, -30.631, 0.0, 0.0, 0.0], [40.18, -220.672, -20.638, 0.0, 0.0, 0.0], [42.45, -114.815, -10.438, 0.0, 0.0, 0.0], [-7.895, -179.8, -22.734, 0.0, 0.0, 0.0]]

    skip_run = False
    for tar in targets:
        if len(tar) != 6:
            print(f'!!WARNING!! tar is incorrect length. {tar}')
            skip_run = True

    if not skip_run:
        force_target_run(targets, zcontact=True, easeon=20, easeoff=30)

    robot.waitmove()
    robot.set_DO(settings.angle_grinder_pin, False)
    robot.movej(settings.home_joints, speed=25)
    robot.movej(settings.picture_joints, speed=25)
    mem.generator1_target_pairs = [] # ## change var
    robot.waitmove()
...
def SelectProgram(p):
    ...
    # elif p==1: test_single_contour_run()
    elif p==100:
